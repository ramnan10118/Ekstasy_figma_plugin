...
export interface CacheEntry {
  status: CacheStatus;
  input: string;
  result: TextIssue[];  // Store full TextIssue objects (no abbreviations)
  meta: {
    model: string;
...
export class CacheManager {
  ...
  private readonly currentPromptVersion = '1.0';
  ...
  private estimateTokensSaved(text: string): number {
    // Rough estimation: system prompt + user prompt + text length
    // This is saved per cache hit since we don't make the API call
    const systemPromptTokens = 220; // Original prompt size
